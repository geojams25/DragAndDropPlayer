...
{
    //ScopedPointer<XmlElement> xmlState(getXmlFromBinary(data, sizeInBytes));//old
    std::unique_ptr<XmlElement> xmlState(getXmlFromBinary(data, sizeInBytes));//new
...
void AudioFilePlayerProcessor::loadFileIntoTransport(const File& audioFile)
{
    ...
    AudioFormatReader* reader = formatManager.createReaderFor(audioFile);//new
